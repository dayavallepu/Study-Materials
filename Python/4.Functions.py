...
val1 = [1, 2, 3, 4, 5, 6]
filtered_odd = list(filter(lambda x: x % 2!=0, val1))
print(filtered_odd)

# Using filter() to filter out numbers greater than 50 from a list \
# generated using range() and convert the result to a list.
...
def fib(n):
    a, b = 0, 1  # Initialize variables 'a' and 'b' to store the first two Fibonacci numbers.
    for i in range(n):  # Iterate 'n' times to generate 'n' Fibonacci numbers.
        yield a  # Yield the current Fibonacci number.
        a, b = b, a + b  # Update 'a' and 'b' to calculate the next Fibonacci number.
