...
#r+ mode it read first and write after 
f=open('Alarmtext.txt','r+')
print(f.read())
print(f.write('\n hii im daya')) #\n added new line here
f.close()

# w+ mode it write first and read after
f=open('Alarmtext.txt','w+')
f.write('hii im daya')
f.seek(0)# file pointer change it read from starting
print(f.read())
f.close()

#a+ mode its append a input first and read after

f=open('Alarmtext.txt','a+')
f.write('\n hii im daya')
f.seek(0)
print(f.read())
f.close()
...
if x < 18:
    ...
    print('you entered: ', x)
#here one more example of exceptional handling
...
def get_age():
    ...
    try:
        age  = int(age_str)
        if age < 0:
            raise InvalidInputError('Age is less than 0')
        return age
    except ValueError:
        raise InvalidInputError('Invalid age')
